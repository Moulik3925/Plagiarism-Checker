            ...
            for (auto match_indice : match_indices)
            {
                ...
                if(match75_found)
                {
                   if(prev_file != NULL) 
                   {
                    ...
                    plagged_submissions.insert(__submission);
                    // if(_submission->student != NULL) __submission->student->flag_student(_submission);
                    // if(_submission->professor != NULL) __submission->professor->flag_professor(_submission);
...
void plagiarism_checker_t::continuous_worker(){
    while (true) {
        ...
        // Lock and wait for tasks
        {
            ...
            cv.wait(lock, [] { return !tasks.empty() || stopThread; });

            if (stopThread && tasks.empty()) break;
...
void plagiarism_checker_t::add_submission(std::shared_ptr<submission_t> __submission)
{
    ...
    task_queue.push([]{work(__submission);});
}
